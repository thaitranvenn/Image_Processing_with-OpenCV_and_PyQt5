...
class LoadQt(QMainWindow):
    def __init__(self):
        ...
        # Chương 3
        self.actioAnhXam.triggered.connect(self.grayImage)
    ...
    def openImage(self):
        fname, filter = QFileDialog.getOpenFileName(
            self, 'Open Image', "", "PNG(*.png);;JPEG(*.jpg *.jpeg);;All Files(*.*)")
        if fname:
            self.loadImage(fname)
            filepath = fname.format()
            name_tuple = os.path.splitext(os.path.basename(filepath))
            file_extension = name_tuple[1]
        else:
            print("Invalid Image")
        print("The format of the image:", file_extension)
